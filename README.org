...
We narrowed this list to [[file:./docs/assignments/week2-game-ideas.md][two candidates]]: /Puzzles/ and /Artillery/ on the basis of technical feasibility and the SDTs covered. From here, we developed a paper prototype for both. As a team we playtested both paper prototypes and made our final selection using a weighted decision matrix (Pugh, 1991) with a criteria derived from the project brief, SDTs and the paper prototype session.

Scores were given on 1-5 scale  and /[[file:./docs/assignments/week3-paper-prototype/idea-A/demo(CAT).mp4][Artillery]]/ was ultimately selected as the winner. We felt the adversarial component of /Artillery/ produced more social and engaging play than /Puzzle/. We also thought the design possibilities: weapon variety, terrain, opponent strategy etc. were more open to development. This was important as we knew we had to develop two 'twists'.
...
*Branching*
We seelected a standard trunk-based git strategy using short-lived feature branches. =Main= is always deployable (we hope) and feature branches follow =feature/<issue-number>-short-description= (or =Fix/…= for bug fixes). At the time of writing the repo has four live branches. The development branch of this report is =report= and is rebased periodically from =Main=.
...
#+TITLE: Merchant Fighters Demo Deck
#+AUTHOR: Group 16
...
- *Title*: MERCHANT FIGHTERS
- *Subtitle*: A two-player artillery due wehre the lecturers fight back
- *Footer*: Group 16,  COMSM0166, University of Bristol

** Speaker
Welcome. Merchant Fighters is a two-player artillery game set inside the Merchant Venturers building, where the lecturers who teach us square off against each other. Last HP standing wins.


* Slide 2: The Novel Idea
:PROPERTIES:
:END:

1. *Continuous, two-player input.* We replaced the turn-based state machine with parallel,  both player and AI aim, charge and fire dynamically
2. *Reflective projectile physics.* Shots ricochet off walls and boundaries instead of detonating on impact creating chaos, banked shot and rich strategy.
3. *Set in Merchant Venturers.* Four lecturers (John, Kira, Mat, Jo) battle across two art routes: a Fantasy castle and a Modern Linux lab.

** Speaker
The artillery genre dates back to 1965, where it was Text-based, turn-by-turn. We extended it in two ways: continuous player-AI input replacing the turn-based loop and reflective projectile physics so shots ricochet rather than detonate. The whole thing is set inside Merchant Venturers, with our lecturers as the competitors.



* Slide 3: How It Plays
:PROPERTIES:
:END:

Easy and Medium drop you into the Fantasy Route,  a Western-fantasy castle with bow-and-arrow combat. Hard takes you into the Modern Route: a contemporary computer lab with paper...with anything else?

** Speaker
Players pick a difficulty, which routes them through one of two art styles: Fantasy castle for Easy and Medium, Modern computer lab for Hard. Same four lecturer-characters in both, drawn twice so the aesthetic carries through from character select to the battle screen.


* Slide 4: Challenge 1: Continuous FSM
:PROPERTIES:
:END:

** On-slide
- *Eyebrow*: Challenge 1
- *Title*: Continuous-input AI state machine

** Speaker
First challenge: making artillery feel dynamic... add some stuff from helen

* Slide 5: hallenge 2: Ricochet Physics
:PROPERTIES:
:END:

*Three part process*, per frame

1. *Integrate.* Explicit Euler with gravity and a horizontal wind term, dt-clamped at 33 ms so tab-focus jumps don't tunnel through walls.
2. *Broad phase.* A 128-px uniform spatial grid prunes static-body candidates to the cells the projectile actually overlaps.
3. *Narrow phase.* Circle-vs-AABB intersection returns a contact point and a surface normal, then v' = v − 2(v·n̂)n̂.

** Speaker
Second challenge: the ricochet. Three-stage pipeline every frame, we integrate position with gravity and wind, broad-phase collision check against a spatial grid, narrow-phase circle-vs-AABB returning a surface normal. The reflection itself is the standard v minus twice v dot n times n. The aim preview runs the same integrator so the dotted arc on screen helps the user fire.


* Slide 6: Meet the Team
:PROPERTIES:
:TIMING: ~15s per member, ~90s total
:NOTE: Duplicate this slide for each of the 6 team members
:END:

** On-slide (Josh's example)
- *Eyebrow*: Meet the team
- *Title*: Josh Thompson:  Documentation

Some of the things I worked on:
- Project documentation: =CONTRIBUTING.md=, ADRs and  the report
- puzzle-game concept and the continuous-FSM design
- heuristic evaluation,  NASA-TLX & SUS analysis
...
- *Title*: Pragmatic, evidence-led process

1. *Empirical method choices*
2. *Trunk-based branching.*
3. *Docs-as-code.*
4. *Metrics .*
** Speaker
On process,  we tried to be evidence-led. priortising good dev-ops practices. Trunk-based git, conventional commits, ADRs. We experimented with literate programming for the report The report. 67 issues, 91% completion, 84 commits.
...
** On-slide: Did it work?
- *Title*: Heuristic review + NASA-TLX + SUS

- *78.8*:  SUS score (Hard). 'Good' band; sessions reaching 'excellent'.
- *p = 0.043*: Hard > Easy on Raw TLX. Mann-Whitney U, one-sided.
- *12 / 9 / 3*: Heuristic findings. Total / negative / positive (Nielsen).

Caption: n=24 playtesters across two sessions. Both difficulties sit in the 'good' band on SUS; Hard significantly raised cognitive load on Raw TLX without raising frustration.

** Speaker
We ran Nielsen's heuristics:  twelve findings, nine negative, three positive and a NASA-TLX plus SUS playtest with 24 respondents. SUS landed at 78 across both modes, comfortably in the 'good' band. Raw TLX showed Hard significantly more demanding than Easy, but frustration stayed near floor in both, meaning the harder route raised cognitive load without irritating people.
...
*Horizon 1: Now: exams*
- Address H7 cluster from heuristic review
- Sound design pass

*Horizon 2: Summer 2026*
- Automated unit tests on simulation classes
- CI/CD pipeline with auto-deploy to Pages
- Counter-balanced playtest, larger n

*Horizon 3L The distant future*
- Online multiplayer over WebRTC
- Procedural arenas beyond Merchant Venturers
- Tournament mode and persistent leaderboard

** Speaker
Three horizons. Near-term we want to address the heuristic findings, add sound, and ship the accessibility toggle. Mid-term: automated tests, CI/CD, a larger playtest. Long-term: online multiplayer, procedural arenas, persistent leaderboards
