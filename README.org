...
Our shortlist of ideas (below) is not exhuastive: missing wonderful genres include economic reasoning (/Stardew Valley/), moral choice (/80 Days/) and rhythm (/Beat Sabre/). It is however broad enough to cover the major psychological challenge types relevant to games.
...
We narrowed this list to two candidates:  /Puzzles/ and /Artillery/ on the basis of technical feasibility and the SDTs covered. From here, we developed a paper prototype for both.

 [[file:./docs/assignments/week2-game-ideas.md][week2-game-ideas.md]]
As a team we playtested both paper prototypes and made our final selection using a weighted decision matrix (Pugh, 1991) with a criteria derived from the project brief, SDTs and the paper prototype session.

- [[file:./docs/assignments/week3-paper-prototype/idea-A/demo(CAT).mp4][Artillery]]

Scores were given on 1-5 scale  and /Artillery/ was ultimately selected. We thought the adversarial component of /Artillery/ produced more social and engaging play than /Puzzle/. We also thought the design possibilities: weapon variety, terrain, opponent strategy etc. were more open to development. This was important as we knew we had to develop two 'twists'.
...
** DONE Workflow
Our way of working was codified in [[file:CONTRIBUTING.md][=docs/CONTRIBUTING.md=]] at the start of the project and briefly summarised, here. We opted to take an empirical approach in selecting processes. Despite the popularity of many software focussed project management methodologies and in particular those related to the Agile Manifesto (Scrum, SAFe and Kanban), empirical evidence of their value is uncertain.

For example, SAFe has not shown, in any rigorous sense, to outperform alternatives even in the enterprise contexts for which it was designed (Conboy & Caroll, 2019). Systematic reviews of Scrum and Kanban productivity find that the relationship may be positive but the evidence remains limited, composed largely of anecdotal case studies, often from consultancy organisations whose primary business depends on their benefits. (Cardozo et al., 2010; Ahmad et al., 2018).
...
- Decision records: architectural decisions are stored in =docs/adr/= as numbered [[https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions][Nygard-template]] markdown files. See: starting ADR ([[file:adr/0000-use-adrs.md][=docs/adr/0000-use-adrs.md=]]) in place.
