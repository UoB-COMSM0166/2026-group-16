...
Enemy AI design pattern in games.
...
State machines. Usually called finite state machines, or FSM. My favorite type of state machine is what I call a "stack machine," and it's something I use all over the place in my code, in everything from high level AI behavior to menu flow. I write about it extensively here: https://playtank.io/2023/11/12/state-space-prototyping/


Planners. For example a goal-oriented action planner (GOAP) is a type of AI used inbclassic FPS like  F.E.A.R.  In this case, you use a graph where you represent desired game state ("PlayerDead:true") as nodes and AI actions ("ShootWithGun") as edges, and you then use goal-first A* to generate plans. It's a much more dynamic and much more interesting model than BTs.


Utility systems. Actions are evaluated based on circumstances and the action that gets the highest estimated utility in a given moment is the one that gets executed. This is often in simulation games.
