...
/Merchant Fighters/ is a two-player real-time strategy-come-shooter from the well-loved arillery genre. Classically, the artillery format is turn-based with players taking aim along simple ballistic trajectories. Our work extends this in two ways:
- Replacing a turn-based state machine with continuous two-player, AI input
...
*Branching*
Trunk-based development with short-lived feature branches. =main= is always deployable. Feature branches follow =feature/<issue-number>-short-description= (or =Fix/…= for bug fixes). At the time of writing the repo has four live branches; the development branch of this report is =report=, rebased periodically from Main.
...
*Docs as Code*
Every design or planning material is under version control and where possible offered under in plain-text. For example, this report is an example of literate programming: where the source code and the report text and 'tangled', meaning what the project does (source) and why it does (report) evolve together.
